...
export default function Intelligence() {
  const iQ = useQuery({ queryKey: ["intelligence"], queryFn: () => adapter.fetchIntelligence(), refetchInterval: 3000 });
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-12 gap-3">
        <Panel title="Validation Breakdown (NICAI)" className="col-span-12 lg:col-span-5">
          {valQ.data && (
            <ValidationDonut
        ...
        <Panel title="Top Reasons (Last 100)" className="col-span-12 lg:col-span-7" noPad>
      ...
      <Panel title="Intelligence Events" noPad>
        <table className="w-full text-sm">
          ...
          <tbody className="font-mono">
            {rows.map((r, i) => (
              <tr key={r.trace_id + i} className="border-t border-line/60 hover:bg-bg-2/40">
                ...
                <td className="px-4 py-2"><ValidationChip validation={r.validation} /></td>
                <td className="px-4 py-2 text-fg-1">{r.reasons.join(", ")}</td>
...
function ReasonList({ rows }: { rows: { reasons: string[]; validation: string }[] }) {
  ...
  return (
    <ul className="divide-y divide-line/60">
      {sorted.map(([reason, count]) => (
        <li key={reason} className="px-4 py-2.5">
          <div className="flex items-center justify-between text-xs">
            <span className="font-mono text-fg-1">{reason}</span>
            <span className="font-mono tabular-nums text-fg-0">{count}</span>
          </div>
